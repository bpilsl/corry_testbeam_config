...
def parse_args():
    ...
    add_arg('--file', type=str, default='export.csv', help='path to csv file')
...
if __name__ == '__main__':
    ...
    if args.framework.lower() == 'corry':
        if args.run:
            os.system(f'analyze.py -r {args.run} -g {params[0][0]} -n {args.n}')
        elif args.start and args.stop:
            for i,run in enumerate(range(int(args.start), int(args.stop)+1)):
                os.system(f'analyze.py -r {str(run)} -g {params[i][0]} -n {args.n}')
